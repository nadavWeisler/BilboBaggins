        ...
        plugin.trial = function(display_element, trial) {
            ...
            setTimeout(function() {
                ...
                const noMaskControl = (len, fade_in_time, max_stimulus_opacity) => {
                    ...
                    if (current_time < len) {
                        let newNoMaskControl = setTimeout(function() {
                            noMaskControl(len, fade_in_time, max_stimulus_opacity);
                        }, trial.mask_duration);
                        setTimeoutHandlers.push(newNoMaskControl);
                    }
                }
                ...
                const rms = (index = 0) => {
                    ...
                    if (hidden) {
                        ...
                        if (trial.mondrianBetweenStimCount > 1) {
                            j = j + 1.0;
                ...
                const end_trial = function() {
                    for (let i = 0; i < setTimeoutHandlers.length; i++) {
                        window.clearTimeout(setTimeoutHandlers[i]);
                    }
